...
- `dhga/trainer.py`: implements real Stage A/B/C/D training loops over unlabeled NIfTI volumes. Labels are not loaded by the trainer except for periodic test-set evaluation. Stage A calls the model baseline path directly. Stage B uses the frozen VoxTell anchor to select foreground, boundary, and optional background patches per case; the number of patches is controlled by `steps_per_volume` and clamped to 2-3. Anchor patch scores are cached per case after the first scoring pass, and background selection is weighted by a simple non-air body ratio. Stage B records student-strong-view foreground/correlation metrics separately from teacher-weak-view disagreement/stability metrics; logs patch metrics plus epoch mean/median/p90/max summaries to TensorBoard; and every configured interval evaluates the 3D test split with GT Dice/IoU by reusing the current model/predictor instead of constructing a second VoxTell before saving an epoch checkpoint. Stage C returns raw boundary-recovery and minimal-transport losses and combines them exactly once in the training step. Stage D separately combines its router-target loss, Stage C auxiliary recovery, equivariance, prompt ranking if enabled, and weak natural minimal transport. Stage D reuses the same-patch teacher output, student output, and teacher SDF for its Stage C auxiliary path to avoid redundant encoder and SDF work. Stage C/D patch ordering is guided by EMA/student pseudo-boundary scores rather than GT. `--init_checkpoint` transfers Stage B to C and Stage C to D; after init loading the EMA teacher is hard-synchronized to the loaded model. `--resume_checkpoint` restores model, optimizer, lightweight EMA, AMP scaler, epoch, global step, and RNG, and same-stage resume is enforced.
- `dhga/evaluation.py`: evaluation-only sliding prediction. It aggregates semantic/app probabilities and low-dimensional projected VoxTell visual features in crop space, applies one SDF geometry correction, restores final, semantic, and appearance masks to original image space, saves NIfTI, and reads GT only for optional metrics including Dice, IoU, precision, recall, FP/FN voxels, volume ratio, connected components, oracle union/intersection, expert complementarity, and disagreement statistics.
