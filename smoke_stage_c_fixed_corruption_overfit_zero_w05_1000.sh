...
FIXED_SAMPLE=".save/dhga/stage_c_v42_fixed_corruption_overfit_resume_1000/fixed_stage_c_sample.pt"

if [[ ! -f "${FIXED_SAMPLE}" ]]; then
  echo "Missing ${FIXED_SAMPLE}."
  echo "Run the fixed-corruption diagnostic once and keep fixed_stage_c_sample.pt before this ablation."
  exit 1
fi
...
python run_3d_dhga.py \
  ...
  --dhga_boundary_recovery_zero_weight 0.5 \
  --dhga_minimal_transport_weight 0.0 \
  --save_dir .save/dhga/stage_c_v42_fixed_corruption_overfit_zero_w05_1000 \
  --stage_c_fixed_sample_path "${FIXED_SAMPLE}" \
  ...
  --stage_c_fixed_overfit_snapshot_steps 0 200 300 500 750 1000 \
